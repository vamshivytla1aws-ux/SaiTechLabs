...
export async function getAuthenticatedStudent() { const session = await readStudentSession(); if (!session) return null; const account = await getDb().studentAccount.findUnique({ where: { id: session.accountId }, include: { student: true } }); if (!account?.isActive || account.studentId !== session.studentId || account.sessionVersion !== session.sessionVersion) return null; return account; }
